...
**Варианты использования сервиса на платформе:** поиск вакансий соискателями, фильтрация нерелевантных кандидатов hr-специалистами.
...
С помощью эмбеддинг моделей (multilingual-e5-large/rubert-base-cased/sbert_large_nlu_ru/LaBSE) векторизуем вакансии и резюме, полученные эмбеддинги храним в векторной базе данных
FAISS.
FAISS выбрана как одна из самых быстрых и удобных векторных баз данных.
...
удаленный формат работы, то вакансии подбираются тоже только удаленные), после этого отбирается топ 10 ближайших c помощью similarity search или maximum marginal relevance.
...
Каждый эксперимент проводился для 10 объектов (вход для каждого эксперимента в таблице одинаковый, выдача релевантных резюме/вакансий разная, так что эти цифры можно сравнивать между собой).
...
Лучше всего модель работает на параметрах FAISS, Similarity search, multilingual-e5-large.
...
элементов в «топе». Так, если из десяти элементов мы угадали только один, то неважно на каком месте он был: на первом,
...
Так, если из трех элементов релевантным оказался только находящийся на последнем месте, то ap@3 = ${1 \over 3}$ * (0 * ${1 \over 1}$ + 0 *
${1 \over 2}$ + 1 * ${1 \over 3}$) = 0.11, если угадали лишь тот, что был на первом месте, то ${1 \over 3}$ * (1 * ${1 \over 1}$ + 0 * ${1 \over 2}$ + 0 * ${1 \over 3}$) = 0.33, а
если угаданы были все, то ${1 \over 3}$ * (1 * ${1 \over 1}$ + 1 * ${2 \over 2}$ + 1 * ${3 \over 3}$) = 1.
...
Производительность решения с конфигурацией FAISS, Similarity search, multilingual-e5-large: ~270 запросов в секунду при
использовании faiss-cpu. На faiss-gpu не проверялось.

Производительность приложения (с выводом данных на странице) зависит еще от длины резюме/вакансии:

| Запрос                                                | Время выдачи |
|-------------------------------------------------------|--------------|
| Поиск вакансий - название должности (без города)      | 0.101 c      |
| Поиск вакансий - название должности (c городом)       | 0.193 c      |
| Поиск вакансий - большое описание резюме (без города) | 1.234 с      |
| Поиск вакансий - большое описание резюме (c городом)  | 2.502 с      |

Описанное время работы справделиво для запуска на CPU. Соответственно большая часть времени уходит на то, чтобы закодировать входной текст с помощью модели-энкодера. Сам FAISS даже на CPU отрабатывает довольно быстро, поэтому решение масштабируемо для использования на большем количестве данных, особенно при использовании GPU.  
2-ю модель для отбора самых релевантных вакансий из тех, что выдала 1-я модель, решили не встраивать, потому что время работы приложения еще увеличится. В FAISS эмбеддинги 2-ой модели быстро закинуть не получилось,
поэтому остановились на начальном варианте с одной моделью. 

#### Выводы

В целом, качество работы сервиса хорошее, о чем свидетельствуют как метрики ранжирования: mean nDCG@5 ≈ 0.76, map@5 ≈ 0.49, так и ручная
оценка работы:
![img.png](img/img_2.png)
![img_1.png](img/img_3.png)

Масштабируемость: работа на миллионах данных за приемлемое время возможна, так как используется FAISS и при добавлении резюме/вакансий мы один раз пересчитываем эмбеддинги во время инициализации векторной БД, то есть время поиска почти не увеличится.
